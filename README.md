...
- **Freshness, when you declare it.** A task with `produces:` is skipped when every product exists and none is older than any `sources:` file — `make`'s contract, from file mtimes. Tasks without `produces:` always run.
